...
openapi-%:
	...
	@docker run --rm                                     \
		...
		openapi-gen                                      \
			--v 1 --logtostderr                          \
			--go-header-file "./hack/license/go.txt" \
			--input-dirs "$(GO_PKG)/$(REPO)/apis/$(subst _,/,$*),k8s.io/apimachinery/pkg/apis/meta/v1,k8s.io/apimachinery/pkg/api/resource,k8s.io/apimachinery/pkg/runtime,k8s.io/apimachinery/pkg/util/intstr,k8s.io/apimachinery/pkg/version,k8s.io/api/core/v1" \
...
gen-crd-protos-%:
	...
	@docker run --rm                                     \
		...
		go-to-protobuf                                   \
			...
			--packages=-k8s.io/api/core/v1,-k8s.io/api/apps/v1,-k8s.io/api/rbac/v1,-kmodules.xyz/custom-resources/apis/appcatalog/v1alpha1,-kmodules.xyz/monitoring-agent-api/api/v1,-kmodules.xyz/offshoot-api/api/v1,kubepack.dev/kubepack/apis/$(subst _,/,$*)
...
install:
	...
	helm install kubepack charts/kubepack \
		...
		$(IMAGE_PULL_SECRETS); \
	kubectl wait --for=condition=Ready pods -n kube-system -l app=kubepack --timeout=5m
