...
import (
	...
	"kubepack.dev/kubepack/pkg/test/ensure"
...
func TestResolveChartOpts(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expect: []Option{
				WithURL("https://example.com/foo-1.2.3.tgz"),
				WithTLSClientConfig("cert", "key", "ca"),
			},
		},
	}
	...
	for _, tt := range tests {
		...
		expect, err := NewHTTPGetter(tt.expect...)
		if err != nil {
		...
		if *(got.(*HTTPGetter)) != *(expect.(*HTTPGetter)) {
