...
func CreateOrPatchBundle(
	...
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating Bundle %s/%s.", meta.Namespace, meta.Name)
...
func PatchBundleObject(
	...
	klog.V(3).Infof("Patching Bundle %s with %s.", cur.Name, string(patch))
...
func TryUpdateBundle(
	...
	err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update Bundle %s due to %v.", attempt, cur.Name, e2)
		return false, nil
	})
